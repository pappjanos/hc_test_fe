				...
				<v-card class="mt-4">
					<v-row class="pa-2">
						<v-col cols="12" md="4" align="center">
							<h2 class="text-h4">Balance: {{ deposit }}</h2>
						</v-col>
		...
		<v-bottom-sheet v-model="sheet" width="100%">
			<v-sheet class="text-center" height="200px">
				...
				<div class="pa-3" v-if="boughtProduct">
					<div>
						You have bought {{ boughtProduct.amount }} piece<span
							v-if="boughtProduct.amount > 1"
							>s</span
						>
						of {{ boughtProduct.productName }}
					</div>
					<br />
					<div>Your change is: {{ boughtProduct.changeAmount }}</div>
				</div>
			</v-sheet>
		</v-bottom-sheet>
	</div>
